...
export default () => {
  const template = `
    ...
    <head>
      ...
      <% if (assets.version) { %><link rel="stylesheet" type="text/css" href="<%= assets.cdnPath %>/auth0-delegated-admin-custom.ui.<%= assets.version %>.css" /><% } %>
    ...
    <body>
      ...
      <% if (assets.version) { %>
      <script type="text/javascript" src="<%= assets.cdnPath %>/auth0-delegated-admin-custom.ui.vendors.<%= assets.version %>.js"></script>
      <script type="text/javascript" src="<%= assets.cdnPath %>/auth0-delegated-admin-custom.ui.<%= assets.version %>.js"></script>
      <% } %>
    </body>
    </html>
    `;
  ...
  return (req, res, next) => {
    ...
    if (clientVersion) {
      const favIcon =
        config("FAVICON_PATH") ||
        "https://cdn.auth0.com/styleguide/4.6.13/lib/logos/img/favicon.png";
      const cdnPath =
        config("CDN_PATH") ||
        (PR_NUMBER
